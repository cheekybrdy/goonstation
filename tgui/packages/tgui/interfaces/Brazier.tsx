 ...
 * @file
 * @copyright 2025
 * @author Cheekybrdy (https://github.com/cheekbybrdy)
 * @license CC
 */
...
const DeptBox = (props) => {
  ...
  return (
    <Section
      ...
      className={classes([
        'Brazier__BuyBox',
        colour && `Brazier__BuyBox_color_${colour}`,
      ])}
    >
      {set_pantheon &&
        set_pantheon.map((set_pantheon, index) => {
          return (
            <>
              {!set_pantheon && (
                <Button
                  onClick={() => act('assign', { assign: set_pantheon, style: style })}
                  key={set_pantheon}
                  selected={set_pantheon === set_pantheon}
                >
                  {set_pantheon}
                </Button>
              )}
              {set_pantheon && (
                <>
                  ...
                  <Button
                    icon="check"
                    tooltip="Confirm"
                    onClick={() => act('set_pantheon', { save: index + 1 })}
                    pl="10px"
                    mx="0.2rem"
                  />
                  <Button
                    icon="cross"
                    tooltip="Cancel"
                    onClick={() => act('cancel', { apply: index + 1 })}
                    pl="10px"
                    mx="0.2rem"
                    mr="1rem"
                  />
    ...
    <Window width={620} height={780}>
      <Window.Content scrollable>
        <Section>
          ...
          {mode === 'selection' && (
            <>
              <h1>Pantheon Conjurationt:</h1>
              <em>
                Please use the security record computer to modify entries.
              </em>
              <Box my="0.5rem" dangerouslySetInnerHTML={{ __html: selection }} />
