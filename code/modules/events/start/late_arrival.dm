/datum/random_event/start/arrivelate
	...
	event_effect(var/source)
		...
		for(var/mob/living/carbon/human/H as anything in mobs)
			if(!H.mind && ROLE_NUKEOP || ROLE_NUKEOP_COMMANDER || ROLE_SALVAGER || ROLE_WIZARD && H.traitHolder.hasTrait("partyanimal") || H.traitHolder.hasTrait("sleepy") || H.traitHolder.hasTrait("pilot"))
				return
